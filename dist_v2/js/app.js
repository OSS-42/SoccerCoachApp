...
const DragState = {
    ...
    isDragging: false,      // toggled when movement exceeds threshold
...
function touchStart(e) {
    ...
    // record initial position for tap detection
    DragState.initialX = touch.clientX;
    DragState.initialY = touch.clientY;
...
function touchMove(e) {
    ...
    // determine if user has moved enough to count as drag
    const dx = Math.abs(touch.clientX - DragState.initialX);
    const dy = Math.abs(touch.clientY - DragState.initialY);
...
function touchEnd(e) {
    ...
    const dx = Math.abs(touch.clientX - DragState.initialX);
    const dy = Math.abs(touch.clientY - DragState.initialY);
    const wasTap = !DragState.isDragging && dx < 10 && dy < 10;

    // Remove clone regardless
    if (clone) {
    ...
    if (wasTap) {
        ...
        handleTapPlayer({currentTarget: draggedElement, stopPropagation: () => {}});
        draggedElement = null;
        DragState.reset();
        return;
    }
...
function showScreen(screenId) {
    ...
    if (screenId === 'team-setup') {
        ...
        renderPlayersList(); // Render players list when team-setup is shown
    }
...
function updateDeletePlayerRibbon() {
    TeamSetupScreen.updateDeletePlayerRibbon();
}

function closeWarningRibbon() {
    TeamSetupScreen.closeWarningRibbon();
}

function openDeletePlayersDialog() {
    TeamSetupScreen.openDeletePlayersDialog();
}

function closeConfirmDeleteDialog() {
    TeamSetupScreen.closeConfirmDeleteDialog();
}

function confirmDeletePlayers() {
    TeamSetupScreen.confirmDeletePlayers();
}
...
function editPlayer(playerId) {
    TeamSetupScreen.editPlayer(playerId);
}

function closeEditPlayerDialog() {
    TeamSetupScreen.closeEditDialog();
}

function savePlayerEdit(playerId) {
    TeamSetupScreen.saveEditedPlayer(playerId);
}
...
// Formation Setup
// Delegated to FormationScreen
function renderFormationSetup() {
    FormationScreen.renderFormationSetup();
    return;
}

// Keepfor backwards compatibility
function _OLD_renderFormationSetup() {
...
// Update viewReport to include formation - Delegated to ReportService
function viewReport(gameId) {
    ReportService.viewReport(gameId);
}

function closeDetailedReport() {
    ReportService.closeDetailedReport();
}

// Update exportReport to ensure formation is styled for PDF
function exportReport(gameId, format) {
    ReportService.exportReport(gameId, format);
}
...
function exportTeamData(format = 'json') {
    // If pdf requested, generate a printable roster instead
    if (format === 'pdf') {
        const title = `Team Roster - ${appState.teamName}`;
        let html = `
            <h2>${title}</h2>
            <table style="width:100%;border-collapse:collapse;font-family:Arial,sans-serif;font-size:12px;">
                <thead>
                    <tr>
                        <th style="border:1px solid #ddd;padding:4px;text-align:left;">#</th>
                        <th style="border:1px solid #ddd;padding:4px;text-align:left;">Name</th>
                        <th style="border:1px solid #ddd;padding:4px;text-align:left;">Position</th>
                    </tr>
                </thead>
                <tbody>
        `;
        appState.players.forEach(p => {
            html += `<tr>
                        ...
                        <td style="border:1px solid #ddd;padding:4px;">${p.position}</td>
                    </tr>`;
        });
        html += `</tbody></table>`;

        // create iframe print exactly like reports
        ...
        doc.write(`
            ...
            <p style="margin-top:30px;text-align:center;color:#666;">Generated by Soccer Coach Tracker &#8211; ${new Date().toLocaleString()}</p>
            </body></html>
        `);
        doc.close();
...
// Delegated to StatisticsService
function calculatePlayerStatistics(startDate = null, endDate = null) {
    return StatisticsService.calculatePlayerStatistics(startDate, endDate);
}

// Clear date filter function - Delegated to StatisticsService
function clearDateFilter() {
    StatisticsService.clearDateFilter();
}

// Update statistics period info and reports counter - Delegated to StatisticsService
function updateStatsPeriodInfo(startDate, endDate, filteredGames) {
    StatisticsService.updateStatsPeriodInfo(startDate, endDate, filteredGames);
}

// Render player statistics table - Delegated to StatisticsService
