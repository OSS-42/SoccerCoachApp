...
describe('i18n', () => {
  it('translates and interpolates in French', () => {
    ...
    expect(t('cannotActRed')).toBe('Red card — no actions available')
    expect(t('confirmPositionFirst')).toBe('Confirm position first — then start game clock')
    ...
    expect(t('roleParent')).toBe('Spectator')
    expect(t('roleParentHint')).toBe('Follow one player')
    expect(t('switchToParent')).toBe('Switch to spectator')
    expect(t('replayTutorialCoach')).toBe('Replay coach tutorial')
    expect(t('replayTutorialParent')).toBe('Replay spectator tutorial')
    setLocale('fr')
    expect(t('roleParent')).toBe('Spectateur')
    expect(t('switchToParent')).toBe('Passer en mode spectateur')
    expect(t('replayTutorialParent')).toBe('Revoir le tutoriel spectateur')
    expect(t('cannotActRed')).toBe('Carton rouge — aucune action possible')
    expect(t('confirmPositionFirst')).toBe('Confirmez d’abord la position — puis lancez le chronomètre')
    setLocale('en')
  })
})
